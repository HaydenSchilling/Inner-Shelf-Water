...
load E:\AAUNSWharddrives\bkhome1stMay07\matlab\SS\patrick\matfiles\hydro_data.mat
...
o2satres = 70:40/masterres:110;
nres = 0:16/masterres:16;
pres = 0:1/masterres:1;
sres = 0:16/masterres:16;
saltres = 35:0.7/masterres:35.7;
tempres = 11:10/masterres:21;
...
%  colormap(jet) 
  ...
  hgt= (1.0 - bot -top -(rows-1)*vspc)/rows;
        ...
        set(gca,'ydir','reverse','ylim',[0 depth(ii)],'ytick',0:50:300,'xlim',[min(xdist) max(xdist)],'xtick',[-15:5:15]);
        %clabel(ch1,h1,35.4:0.1:35.7,'color','w');
        ylabel('Pressure (dbar)');
        set(gca,'Xticklabel','');
        text(0,-40*depth(ii)/300,'Fluorescence (dimensionless)','HorizontalAlignment','center');
        ...
        set(gca,'Yticklabel','');
        set(gca,'Xticklabel','');
        text(0,-40*depth(ii)/300,'Nitrate (mmol m^{-3})','HorizontalAlignment','center');
        if (ii==4)
          set(gca,'xdir','reverse');
...
eval(['dens = sw_dens(Section0',num2str(ii),'.temperature,Section0',num2str(ii),'.salinity,Section0',num2str(ii),'.pressure''*ones(1,size(Section0',num2str(ii),'.temperature,2)))']);
  ...
  % eval(['sw02 = sw_satO2(Section0',num2str(ii),'.salinity,Section0',num2str(ii),'.temperature).*44.6628./(1.0 + pdens/1000).*(1+dens/1000)']);
        ...
        ylabel('Pressure (dbar)');
        set(gca,'Xticklabel','');
        text(0,-40*depth(ii)/300,'O_2: CTD (% saturation) and Bottle (mmol m^{-3})','HorizontalAlignment','center');
        plot(xdist,ones(size(xdist)),'v','Color','k','MarkerFaceColor','auto','MarkerSize',6,'MarkerFaceColor','k');
        if (ii==4)
          set(gca,'xdir','reverse');
        ...
        set(gca,'Yticklabel','');
        set(gca,'Xticklabel','');
        text(0,-40*depth(ii)/300,'Phosphate (mmol m^{-3})','HorizontalAlignment','center');
        if (ii==4)
          set(gca,'xdir','reverse');
        ...
        ylabel('Pressure (dbar)');
        text(0,360*depth(ii)/300,'cross front distance [km]','HorizontalAlignment','center');
        %text(0,-40*depth(ii)/300,'Bottle Oxygen (mmol m^{-3})','HorizontalAlignment','center');
        text(0,-40*depth(ii)/300,'Salinity (PSU) and Temperature (^oC)','HorizontalAlignment','center');
        plot(xdist,ones(size(xdist)),'v','Color','k','MarkerFaceColor','auto','MarkerSize',6,'MarkerFaceColor','k');
        if (ii==4)
          set(gca,'xdir','reverse');
        ...
        set(gca,'Yticklabel','');
        text(0,360*depth(ii)/300,'cross front distance [km]','HorizontalAlignment','center');
        text(0,-40*depth(ii)/300,'Silicate (mmol m^{-3}) and \sigma_t','HorizontalAlignment','center');
        if (ii==4)
          set(gca,'xdir','reverse');

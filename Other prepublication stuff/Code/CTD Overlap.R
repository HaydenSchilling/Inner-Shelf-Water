...
CTD	<- read.csv("data/MNF_SS200408_ctd_trawler/MNF_SS200408_ctd_trawler.csv", header=TRUE, sep=",", dec=".", fill=TRUE)
CTD_points <- CTD
...
#define extent same as Jase's plot
extent<- c(153.35,153.75,-30.004 , -29.996)

#plot
plot(extent(extent), ylab=expression(Latitude*phantom(0)*degree*S),xlab=expression(Longitude*phantom(0)*degree*E), col="white",main=" ")
plot(OPC_points, col="blue", add=T, pch=16, cex=0.5)
...
OPC_CTD	<- read.csv("data/OPC_CTD 120419.csv", header=TRUE, sep=",", dec=".", fill=TRUE)
...
OPC_CTD_full$site2 <- plyr::revalue(OPC_CTD_full$site, c("EvansHead"="EH",
                                           ...
                                           "DiamondHead"="DH",
                                           "CapeByron"="CB"))
#write.csv(OPC_CTD_full, file="data/OPC_CTD discrepency 120419.csv", row.names=FALSE)

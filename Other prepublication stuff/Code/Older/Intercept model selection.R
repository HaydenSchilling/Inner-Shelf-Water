...
df2<- df %>% select(NBSSIntercept,Temp,fluorescence,GA_depth2,Dbin,site2, Lon)
...
#try a basic lm
M1<- lm(NBSSIntercept~Temp+fluorescence+GA_depth2+Dbin+site2,
        data=df3)
...
#try GLM with Gamma
M2<- glm(NBSSIntercept~Temp+fluorescence+GA_depth2+Dbin+site2,
...
M3 <- glm(NBSSIntercept~poly(Temp,3)+poly(fluorescence,3)+poly(GA_depth2,3)+Dbin+site2,
          data=df3,family=gaussian)
E3 <- rstandard(M3)
df3$E3 <- E3
...
M4 <- gam(NBSSIntercept~s(Temp)+s(fluorescence)+s(GA_depth2)+Dbin+site2,
          data=df3,family=gaussian, method="REML") #with cross validation to select optimal amount of smoothing (GAM book pg 47)
...
#try Gaussian
M5 <- gam(NBSSIntercept~s(Temp)+s(fluorescence)+s(GA_depth2)+Dbin+site2,
...
#M6<- glm(NBSSIntercept~Temp+fluorescence+GA_depth2+site2,Dbin,
#  data=df3,family=gaussian, correlation = corGaus(form = ~ Dbin+Lon| site2))

M7 <- gam(NBSSIntercept~
            s(Temp, by = Dbin)+
...
plot(cooks.distance(M10), type="h", ylim=c(0,1))
df3$cooks_M10 <- cooks.distance(M8)
df5<- df3 %>% dplyr::filter(cooks_M10 < 0.1)

M10b <- gam(NBSSIntercept~
             s(Temp)+
             s(fluorescence)+
             s(GA_depth2)+
             site2+Dbin,
           data=df5,family=gaussian, method="REML",
           correlation = corGaus(form = ~ Dbin+Lon| site2)) 

pdf('3plots/NBSSIntercept.pdf', width = 6, height =8)
par(mfrow=c(3,2))
visreg(M10b, "Temp", scale="response", line=list(col="black", lwd=1), rug=F)
visreg(M10b, "fluorescence", scale="response", line=list(col="black", lwd=1), rug=F)
visreg(M10b, "GA_depth2", scale="response", line=list(col="black", lwd=1), rug=F)
visreg(M10b, "site2", scale="response", line=list(col="black", lwd=1), rug=F)
visreg(M10b, "Dbin", scale="response", line=list(col="black", lwd=1), rug=F)
dev.off()

#w
M11 <- gam(NBSSIntercept~
             s(Temp, by=site2)+

...
df2<- df %>% select(GeoMn,Temp,GA_depth2,Dbin,site2, Lon)
...
#try a basic lm
M1<- lm(GeoMn~Temp+GA_depth2+Dbin+site2,
        data=df3)
...
#try GLM with Gamma
M2<- glm(GeoMn~Temp+GA_depth2+Dbin+site2,
...
M3 <- glm(GeoMn~poly(Temp,3)+poly(GA_depth2,3)+Dbin+site2,
          data=df3,family=Gamma(link="log"))
E3 <- rstandard(M3)
df3$E3 <- E3
...
M4 <- gam(GeoMn~s(Temp)+s(GA_depth2)+Dbin+site2,
...
#try Gaussian
M5 <- gam(GeoMn~s(Temp)+s(GA_depth2)+Dbin+site2,
...
pdf('3plots/GeoMn_4sites.pdf', width = 7, height =6)
par(mfrow=c(2,2))

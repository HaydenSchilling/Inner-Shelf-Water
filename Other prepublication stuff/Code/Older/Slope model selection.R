...
df2<- df %>% select(NBSSSlope,Temp,fluorescence,GA_depth2,Dbin,site2, Lon)
...
#try a basic lm
M1<- lm(NBSSSlope~Temp+fluorescence+GA_depth2+Dbin+site2,
        data=df3)
...
#try GLM with Gamma
M2<- glm(NBSSSlope~Temp+fluorescence+GA_depth2+Dbin+site2,
...
M3 <- glm(NBSSSlope~poly(Temp,3)+poly(fluorescence,3)+poly(GA_depth2,3)+Dbin+site2,
          data=df3,family=gaussian)
E3 <- rstandard(M3)
df3$E3 <- E3
...
M4 <- gam(NBSSSlope~s(Temp)+s(fluorescence)+s(GA_depth2)+Dbin+site2,
...
#try Gaussian
M5 <- gam(NBSSSlope~s(Temp)+s(fluorescence)+s(GA_depth2)+Dbin+site2,
...
#M6<- glm(NBSSSlope~Temp+fluorescence+GA_depth2+site2,Dbin,
...
#pdf('3plots/NBSSSlope.pdf', width = 6, height =8)
par(mfrow=c(3,2))

...
df2<- df %>% select(NBSSIntercept,Temp,GA_depth2,Dbin,site2, Lon)


################################################
# with CB and withoutfluorescence
################################################

df3 <- na.omit(df2) 

#try a basic lm
M1<- lm(NBSSIntercept~Temp+GA_depth2+Dbin+site2,
        data=df3)
...
#try GLM with Gamma
M2<- glm(NBSSIntercept~Temp+GA_depth2+Dbin+site2,
...
M3 <- glm(NBSSIntercept~poly(Temp,3)+poly(GA_depth2,3)+Dbin+site2,
          data=df3,family=gaussian)
E3 <- rstandard(M3)
df3$E3 <- E3
...
M4 <- gam(NBSSIntercept~s(Temp)+s(GA_depth2)+Dbin+site2,
...
#try Gaussian
M5 <- gam(NBSSIntercept~s(Temp)+s(GA_depth2)+Dbin+site2,
...
pdf('3plots/NBSSIntercept_4sites.pdf', width = 7, height =6)
par(mfrow=c(2,2))

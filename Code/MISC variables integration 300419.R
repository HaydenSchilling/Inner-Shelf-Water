# Hayden can not run as he does not have the correct Bathy file.
...
#read main data
OPC_CTD_HYD_ADP	<- read.csv("data/OPC_CTD_HYD_ADP 300419.csv", header=TRUE, sep=",", dec=".", fill=TRUE)

#read discrepancy data (this is created at the end of 'CTD overlap')
DIS	<- read.csv("data/OPC_CTD discrepency 120419.csv", header=TRUE, sep=",", dec=".", fill=TRUE, na.strings = c("", " "))
#rename site in OPC
...
###################################################################################
#Bathy data
load(file = paste("data/GA_bathy.rda"))
...
#write.csv(Data, file="1data/IntegratedData_300419.csv", row.names=FALSE)

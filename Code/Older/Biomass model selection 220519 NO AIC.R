...
pdf('3plots/Biomass_3sites_gamm220519.pdf', width = 6, height =8)
par(mfrow=c(3,2))
visreg(M3$gam, "Temp",print.cond=T)#,scale="response", line=list(col="black", lwd=1), rug=F)
...
#different levels
pdf('3plots/Biomass_plots at diff levels.pdf', width = 6, height =8)
par(mfrow=c(4,2))
visreg(M3$gam, "Temp",print.cond=T, cond=list(site2="EH"), main="site = EH")
visreg(M3$gam, "Temp",print.cond=T, cond=list(site2="NS"), main="site = NS")
visreg(M3$gam, "Temp",print.cond=T, cond=list(site2="DH"), main="site = DH")

visreg(M3$gam, "Temp",print.cond=T, cond=list(Dbin="0_25"), main="Dbin = 0_25")
visreg(M3$gam, "Temp",print.cond=T, cond=list(Dbin="26_55"), main="Dbin = 26_55")
visreg(M3$gam, "Temp",print.cond=T, cond=list(Dbin="56_85"), main="Dbin = 56_85")
visreg(M3$gam, "Temp",print.cond=T, cond=list(Dbin="85_115"), main="Dbin = 86_115")
dev.off()


pdf('3plots/gamBiomass_plots at diff levels.pdf', width = 6, height =8)
par(mfrow=c(4,2))
visreg(M2, "Temp",print.cond=T, cond=list(site2="EH"), main="site = EH")
visreg(M2, "Temp",print.cond=T, cond=list(site2="NS"), main="site = NS")
visreg(M2, "Temp",print.cond=T, cond=list(site2="DH"), main="site = DH")

visreg(M2, "Temp",print.cond=T, cond=list(Dbin="0_25"), main="Dbin = 0_25")
visreg(M2, "Temp",print.cond=T, cond=list(Dbin="26_55"), main="Dbin = 26_55")
visreg(M2, "Temp",print.cond=T, cond=list(Dbin="56_85"), main="Dbin = 56_85")
visreg(M2, "Temp",print.cond=T, cond=list(Dbin="85_115"), main="Dbin = 86_115")
dev.off()

#cond=list(Temp=16, site2="DH")
par(mfrow=c(3,2))
...
EH <- EH[order(EH$GA_depth2),] 
EH$ID <- seq.int(nrow(EH))
NS <- df2[df2$site2 == "NS" & df2$Depth < 30,]
NS <- NS[order(NS$GA_depth2),] 
NS$ID <- seq.int(nrow(NS))
DH <- df2[df2$site2 == "DH" & df2$Depth < 30,]
DH <- DH[order(DH$GA_depth2),] 
DH$ID <- seq.int(nrow(DH))
CB <- df2[df2$site2 == "CB" & df2$Depth < 30,]
CB <- CB[order(CB$GA_depth2),] 
CB$ID <- seq.int(nrow(CB))

breaks <- c(40,60, 80, 100, 1300)
EH$bathygroup <- as.factor(cut(EH$GA_depth2, breaks=breaks))
NS$bathygroup <- as.factor(cut(NS$GA_depth2, breaks=breaks))
CB$bathygroup <- as.factor(cut(CB$GA_depth2, breaks=breaks))
DH$bathygroup <- as.factor(cut(DH$GA_depth2, breaks=breaks))
...
pdf('3plots/cross shelf boxplots 210519.pdf', width = 9, height =6)
# par(mfrow=c(1,4))
# plot(CB$GA_depth2, col=CB$bathygroup, main = "CB")
# plot(EH$GA_depth2, col=EH$bathygroup, main = "EH")
# plot(NS$GA_depth2, col=NS$bathygroup, main = "NS")
# plot(DH$GA_depth2, col=DH$bathygroup, main = "DH")

...
p_season <- ggplot(plot_dat, aes(x = Month, y = Along_Velocity, lty = Transect)) +  geom_point() +
  ...
  theme(axis.title.x = element_text(face="bold", colour="black", size = 18),
        axis.text.x  = element_text(colour="black", size = 14, angle = 45, vjust=0.5, ), 
        axis.title.y = element_text(face="bold", colour="black", size = 18),
        axis.text.y  = element_text(colour="black", size = 14),
        legend.key.width = unit(3, "line"),
        legend.position=c(.45, .87),
...
ggsave("Other prepublication stuff/plots/velocity seasonality.png", width = 21, height = 14.8, units = "cm", dpi = 600)
ggsave("Other prepublication stuff/plots/velocity seasonality.pdf", width = 21, height = 14.8, units = "cm", dpi = 600)


plot_dat2 <- vel_dat %>% group_by(Transect) %>% 
  summarise(Along_Velocity = mean(V_shore)*-1, n = n(), SD = sd(V_shore), SE = SD/sqrt(n))
plot_dat2
